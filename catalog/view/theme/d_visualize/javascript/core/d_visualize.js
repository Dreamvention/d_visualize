		...
		$.ajax({
			...
			success: function success(json) {
				...
				if (json['redirect']) {
					...
					if (json['error']['warning']) {
						that.setting.$shipping_method_body.prepend('<div class="alert alert-warning">' + json['error']['warning'] + '<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
					}
				} else {
					if (typeof callback == "function") callback();
				}
			...
			success: function success(json) {
				...
				if (json['success']) {
					var insertData = function insertData() {
						var r = $.Deferred();
						$('#content').parent().before('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json['success'] + ' <button type="button" class="close">&times;</button></div>');
						return r;
					};

					var addAnim = function addAnim() {
						$('.alert').addClass('js--slide');
					};

					setTimeout(function () {
						insertData().done(addAnim());
					}, 100);

					$(document).on('click', '.close', function () {
						var that = $(this);

						$(this).closest('.alert').removeClass('js--slide');
						$(this).closest('.alert').addClass('js--slide-out').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function () {
							$(this).removeClass('js--slide-out');
						});

						setTimeout(function () {
							that.closest('.alert').remove();
						}, 1100);
					});
		...
		$.ajax({
			...
			success: function success(json) {
				...
				if (json['zone'] && json['zone'] != '') {
					for (var _i = 0; _i < json['zone'].length; _i++) {
						html += '<option value="' + json['zone'][_i]['zone_id'] + '"';

						if (json['zone'][_i]['zone_id'] == setting.zone_id) {
							html += ' selected="selected"';
						}

						html += '>' + json['zone'][_i]['name'] + '</option>';
					}
				} else {
...
var Product = {
	...
	decreaseQuanityt: function decreaseQuanityt() {
