...
export function getUrlOpencart(url, isDev) {
    ...
    parsed_url.makeUrl = function () {
        parsed_url.query = 'route=' + parsed_url.route + '&user_token=' + parsed_url.user_token;
        if (isDev ) {
            return `${parsed_url.href}api/?${parsed_url.query}`;
        } else {
            return `${parsed_url.href}?${parsed_url.query}`;
        }
    };
    return parsed_url;
}
